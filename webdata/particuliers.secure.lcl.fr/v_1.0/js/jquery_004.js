...
;(function ($, window, document, undefined) {
	...
	function RS(elem, settings, nmbr) {
		...
		this.$sliderWrap        = this.$slider.wrap('<div class="rs-wrap' + nmbr + '" />').parent();      // Elem: Slider wrapper div
		this.$sliderBG          = this.$slider.wrap('<div class="rs-slide-bg' + nmbr + '" />').parent();  // Elem: Slider background (useful for styling & essential for cube transitions)
	...
	RS.prototype = {
        init: function () {
           ...
           // Setup pagination navigation
            if (this.settings['controls'] === 'pagination') this.setPagination();
        ...
        ,setAutoPlay: function () {
            ...
            this.cycling = setTimeout(function () {
            	_this.next();
            	this.$('.rs-page-wrap' + _this.nmbr).find('a').removeClass('active');
            	this.$('.rs-page-wrap' + _this.nmbr).find('a.rs-slide-link-' + _this.currentPlace).addClass('active');
        ...
        ,setPagination: function() {
        	...
        	// <div> wrapper to contain numbers
        	this.$pageWrapper = $('<div class="rs-page-wrap' + _this.nmbr + '" />');
        	this.$pageWrapper.css('width', this.totalSlides*30 +'px');
        	this.$pageWrapper.css('marginLeft', '-' + (this.$pageWrapper.width()/2) + 'px');
        	 
            this.$pageWrap = $(this.$pageWrapper).appendTo(this.$sliderWrap);
            
            // Loop to apply pagination
            for (var i = 0; i < this.totalSlides; i++) {
                var $page = $('<a class="rs-slide-link-'+ i +'" />').attr({'href' : '#'}).html(i+1);
                $($page).appendTo(this.$pageWrap);
            }
            
            // Add active class to starting slide's respective pagination
            $(this.$pageWrapper.find('a')[this.settings['startSlide']]).addClass('active');

            // Listen for click events on thumnails
            this.$pageWrapper.on('click', 'a', function (e) {
                e.preventDefault();
                _this.$pageWrapper.find('a').removeClass('active');
                $(this).addClass('active');


                // Get identifier from pagination class
                var cl = parseInt($(this).attr('class').split('-')[3]);
	...
	$.fn['refineSlide2'] = function (settings) {
		return this.each(function () {
            // Check if already instantiated on this elem
		    if (!$.data(this, 'refineSlide2')) {
                // Instantiate & store elem + string
		        $.data(this, 'refineSlide2', new RS(this, settings, 1));
			}
		});
	}
	...
	$.fn['refineSlide'] = function (settings) {
	    return this.each(function () {
	        ...
	        if (!$.data(this, 'refineSlide')) {
	            // Instantiate & store elem + string
	            $.data(this, 'refineSlide', new RS(this, settings, 2 ));
	        }
	    });
	}
