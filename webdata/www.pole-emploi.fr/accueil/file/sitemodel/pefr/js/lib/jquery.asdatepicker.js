 ...
 *
 * ----------------------------------------------------------
 * Deux modifications pour int�gration sur P�le emploi
 *
 *	1- Mise en commentaire du _setValue lors de l'init pour ne pas avoir de valeur dans l'input au chargement de la page
 *	2- Cr�ation � la vol�e des calendriers au chargement de la page gr�ce � la class .calendar sur l'input
 */
...
(function($) {
    ...
    AsDatepicker.defaults = {
        ...
        date: 'today', //today|Date (dd-mm-yyyy)
    ...
    AsDatepicker.prototype = {
        ...
        _init: function() {
            //init the status
            this.format = this._parseFormat('dd-mm-yyyy');
            this.outputFormat = this._parseFormat(this.options.format || 'dd/mm/yyyy');
			...
			// 1- Mise en commentaire du _setValue lors de l'init pour ne pas avoir de valeur dans l'input au chargement de la page
            if (this.options.date !== 'today') this._setValue();
        },
        _initDate: function() {			
                ...
                case 'string':
                    if (data.length < 5) {
                        ...
                        if (parts.length === format.parts.length) {
                            for (var i = format.parts.length-1; i >=0; i--) {
        ...
        _buildDays: function(i) {
            ...
            for (var j = 0; j < 7; j++) {
                html += '<span><abbr title="' + LABEL[this.options.lang].days[j] + '">' + LABEL[this.options.lang].daysShort[j] + '</abbr></span>';
            }
